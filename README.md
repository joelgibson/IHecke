...
for this standard parabolic, use `IHeckeAntiSpherical` on the `HAlg` object:

    > ASph := IHeckeAntiSpherical(HAlg, [1, 2, 3]);
    > ASph;
...
The standard basis is created by calling `IHeckeAntiSphericalStd` on the `ASph` module object:

    > aH := IHeckeAntiSphericalStd(ASph);
    > aH;
